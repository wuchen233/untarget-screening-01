...
def get_header(MSFile): 
    ...
    header = [
                ...
                ['Data points: ' + str(len(MSFile.GetAverageMassList(1,40)[0][0]))],
                ['Mass', 'Intensity']
            ]
...
def get_spectrum(filename):
    ...
    averaged = rawfile.GetAverageMassList(1,40)[0]
    spectrum  = get_header(rawfile)
...
def average_dotraws(inputs, output):
    start = time.time() 
    spectra = []
    spectra_lens = []
    for filename in inputs:
        spectrum = (get_spectrum(filename))
        spectra.append(spectrum)
        spectra_lens.append(len(spectrum))
    for spectrum in spectra: 
        while len(spectrum) < max(spectra_lens):
            spectrum.append('')

    with open(output, 'w') as csvfile:
        csvwriter = csv.writer(csvfile)
        for idx in range(max(spectra_lens)):
            row = []
            for spectrum in spectra: 
    ...
    print('Samples ' + str(inputs)[1:-1] + ' averaged and stored in ' + str(output))
